...
public class BadRequestExceptionMapper implements ExceptionMapper<BadRequestException> {
    ...
    public Response toResponse(BadRequestException exception) {
       ...
       ErrorMessage errorMessageEntity =  new ErrorMessage(Response.Status.BAD_REQUEST.getStatusCode(),
                ErrorTypes.BAD_REQUEST_EXCEPTION.getErrorType(),
                exception.getMessage());

       // Maps Error Status 400 to Response
       return Response.status(Response.Status.BAD_REQUEST).entity(errorMessageEntity).type(MediaType.APPLICATION_JSON).build();
    }
}
